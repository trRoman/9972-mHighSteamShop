...
- Перемешивание товаров внутри категории для каждого визита (детерминированно по cookie‑seed).
...
- Корзина `/cart`:
  ...
  - форма с полями Имя (обязательно), Телефон (маска +7-(XXX)-XXX-XX-XX), Адрес (по умолчанию);
...
- Изображения в каталоге и корзине грузятся лениво (lazy‑load) с плавным появлением и скелетоном‑плейсхолдером.
...
- При загрузке в админке изображения оптимизируются сервером: конвертация в WebP (при наличии `sharp`), ресайз до `1600×1200` (fit: inside), качество ~80. Если `sharp` отсутствует, сохраняется оригинал с корректным расширением.
- В прод‑конфигурации отключён клиентский оптимизатор Next Image (`images.unoptimized: true`) — отдаём уже оптимизированные сервером файлы из `public/products/` для стабильной работы динамических загрузок.
